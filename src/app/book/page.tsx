...
export default function BookPage() {
  ...
  const subtotal = ALL_TIERS.reduce(
  ...
  const tierCount = ALL_TIERS.filter((t) => (qty[t.id] ?? 0) > 0).length;
  ...
  const goNext = async () => {
    ...
    if (step === 2) {
      ...
      setPaymentError("");
      try {
        const items = ALL_TIERS.filter((t) => (qty[t.id] ?? 0) > 0).map(
          (t) => ({ tier_id: t.id, qty: qty[t.id] }),
        );
        ...
        <div className="shell">
          <div className={styles.headerWrap}>
            ...
            <h1 className={`h2 ${styles.title}`}>
              ...
              <br />
              <em>{CURRENT_SESSION.subtitle.toLowerCase()}.</em>
            </h1>
          </div>
